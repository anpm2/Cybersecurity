...
* Mình đã đổi tên để dễ nhận biết với các hàm theo thứ tự từ trong ra ngoài là check format của **flag** (`KCSC{...}`), check vị trí của các số (`digit`), check vị trí của dấu `_`, check vị trí của các chữ cái (`let`) trong mảng `let` và cuối cùng là hàm check các chữ cái còn lại trong mảng `final`.
* 
